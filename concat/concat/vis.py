...
def draw_strokes(data, factor=1, svg_filepath = 'tmp/sample.svg', stroke_width = 3,
    ...
    dwg = svgwrite.Drawing(svg_filepath, size = dims)
    # dwg.add(dwg.rect(insert=(0, 0), size=dims,fill='white'))
    dwg.add(dwg.rect(insert=(0, 0), size=dims))
    lift_pen = 1
    abs_x = 25 - min_x
    abs_y = 25 - min_y
    p = "M%s,%s " % (abs_x, abs_y)
    ...
    dwg.save()
    no_ext = os.path.basename(svg_filepath).split('.')[0]
    png_name = no_ext + '.png'
    png_path = os.path.join(os.path.dirname(svg_filepath), png_name)
    cairosvg.svg2png(url=svg_filepath, write_to=png_path)
...
def write_strokes_to_svg(data, factor=1, svg_filepath = 'tmp/sample.svg', stroke_width = 3,
                square = False, return_res = False, crop = True):
